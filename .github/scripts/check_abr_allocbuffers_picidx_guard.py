...
def check_repo(repo_root):
    ...
    for snippet in REQUIRED_SNIPPETS:
        if snippet not in text:
            failures.append((TARGET.as_posix(), 0, f'missing abr allocBuffers picIdx guardrail: {snippet}'))
    ...
    def has_guarded_picidx_read(block):
        alloc_pos = block.find('m_picIdxReadCnt[pass] = new (std::nothrow) ThreadSafeInteger[m_queueSize];')
        guard_pos = block.find('if (!m_picIdxReadCnt[pass])', alloc_pos if alloc_pos != -1 else 0)
    ...
    if not has_guarded_picidx_read(branch1_text) or not has_guarded_picidx_read(branch2_text):
        failures.append((TARGET.as_posix(), 0, 'AbrEncoder::allocBuffers must guard m_picIdxReadCnt[pass] allocation in both queue setup branches'))

    return failures


def main():
    parser = argparse.ArgumentParser(description='Check AbrEncoder allocBuffers picIdx allocation guards')
    ...
    print('AbrEncoder allocBuffers picIdx guards validated')
