...
REGION_START = 'OPT("ctu-info")'
REGION_END = 'OPT("splitrd-skip")'
...
def check_repo(repo_root):
    ...
    for forbidden in FORBIDDEN_SNIPPETS:
        if forbidden in region:
            failures.append((TARGET.as_posix(), 0, 'forbidden misc-control regression: invalid values must not overwrite prior state'))
            return failures
    for snippet in REQUIRED_SNIPPETS:
        if snippet not in region:
            failures.append((TARGET.as_posix(), 0, f'missing misc-control guardrail: {snippet}'))
    if all(snippet in region for snippet in REQUIRED_SNIPPETS):
        if not has_in_order(
            ...
            failures.append((TARGET.as_posix(), 0, 'Misc control parsing must stage parsed integers and only publish them after the reviewed error gates succeed'))
    return failures


def main():
    parser = argparse.ArgumentParser(description='Check misc control parse safety guardrails')
    ...
    print('Misc control parse safety validated')
