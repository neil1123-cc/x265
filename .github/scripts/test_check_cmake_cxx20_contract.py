...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        compile_commands_helper_source = write_repo(root / 'compile-commands-helper-pass')
        compile_commands_helper_script = compile_commands_helper_source.parent / '.github' / 'scripts' / 'check_compile_commands.py'
        compile_commands_helper_script.write_text('''
        ...
        def normalize_standard_flag(flag):
            ...
            return flag
        ''')
        expect_pass(run_checker(compile_commands_helper_source))
        ...
        split_workflow_standard_source = write_source(root / 'split-workflow-standard-flag-fail')
        split_workflow_root = split_workflow_standard_source.parent
        (split_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (split_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (split_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Build
        run: c++ --std gnu++17 -c probe.cpp
''')
        expect_fail(run_checker(split_workflow_standard_source), 'manual C++ standard flag in workflow/helper')

        split_helper_standard_source = write_source(root / 'split-helper-standard-flag-fail')
        split_helper_scripts = split_helper_standard_source.parent / '.github' / 'scripts'
        split_helper_scripts.mkdir(parents=True)
        (split_helper_scripts / 'helper.sh').write_text('c++ --std= gnu++17 -c probe.cpp\n')
        expect_fail(run_checker(split_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        continuation_workflow_standard_source = write_source(root / 'continuation-workflow-standard-flag-fail')
        continuation_workflow_root = continuation_workflow_standard_source.parent
        (continuation_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (continuation_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (continuation_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Build
        run: |
          c++ --std \\
            gnu++17 -c probe.cpp
''')
        expect_fail(run_checker(continuation_workflow_standard_source), 'manual C++ standard flag in workflow/helper')

        continuation_helper_standard_source = write_source(root / 'continuation-helper-standard-flag-fail')
        continuation_helper_scripts = continuation_helper_standard_source.parent / '.github' / 'scripts'
        continuation_helper_scripts.mkdir(parents=True)
        (continuation_helper_scripts / 'helper.sh').write_text('c++ --std= \\\n  gnu++17 -c probe.cpp\n')
        expect_fail(run_checker(continuation_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        split_helper_standard_pass_source = write_source(root / 'split-helper-standard-pass')
        split_helper_pass_scripts = split_helper_standard_pass_source.parent / '.github' / 'scripts'
        split_helper_pass_scripts.mkdir(parents=True)
        (split_helper_pass_scripts / 'helper.sh').write_text('c++ --std=gnu++20 -c probe.cpp\n')
        expect_pass(run_checker(split_helper_standard_pass_source))

        continuation_helper_standard_pass_source = write_source(root / 'continuation-helper-standard-pass')
        continuation_helper_pass_scripts = continuation_helper_standard_pass_source.parent / '.github' / 'scripts'
        continuation_helper_pass_scripts.mkdir(parents=True)
        (continuation_helper_pass_scripts / 'helper.sh').write_text('c++ --std= \\\n  gnu++20 -c probe.cpp\n')
        expect_pass(run_checker(continuation_helper_standard_pass_source))

        variable_workflow_standard_source = write_source(root / 'variable-workflow-standard-flag-fail')
        variable_workflow_root = variable_workflow_standard_source.parent
        (variable_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (variable_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (variable_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Build
        run: |
          std_prefix='--std='
          std_dialect='gnu++17'
          c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_fail(run_checker(variable_workflow_standard_source), 'manual C++ standard flag in workflow/helper')

        variable_helper_standard_source = write_source(root / 'variable-helper-standard-flag-fail')
        variable_helper_scripts = variable_helper_standard_source.parent / '.github' / 'scripts'
        variable_helper_scripts.mkdir(parents=True)
        (variable_helper_scripts / 'helper.sh').write_text('''
std_flag='--std='
std_flag+='gnu++17'
c++ "$std_flag" -c probe.cpp
''')
        expect_fail(run_checker(variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        variable_cmake_standard_source = write_source(root / 'variable-cmake-standard-flag-fail')
        variable_cmake_scripts = variable_cmake_standard_source.parent / '.github' / 'scripts'
        variable_cmake_scripts.mkdir(parents=True)
        (variable_cmake_scripts / 'helper.sh').write_text('''
std_key='STANDARD'
std_value='17'
cmake "-DCMAKE_CXX_${std_key}=${std_value}" -S source -B build
''')
        expect_fail(run_checker(variable_cmake_standard_source), 'manual C++ standard flag in workflow/helper')

        variable_helper_standard_pass_source = write_source(root / 'variable-helper-standard-pass')
        variable_helper_pass_scripts = variable_helper_standard_pass_source.parent / '.github' / 'scripts'
        variable_helper_pass_scripts.mkdir(parents=True)
        (variable_helper_pass_scripts / 'helper.sh').write_text('''
std_prefix='--std='
std_dialect='gnu++20'
c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_pass(run_checker(variable_helper_standard_pass_source))

        local_variable_helper_standard_source = write_source(root / 'local-variable-helper-standard-flag-fail')
        local_variable_helper_scripts = local_variable_helper_standard_source.parent / '.github' / 'scripts'
        local_variable_helper_scripts.mkdir(parents=True)
        (local_variable_helper_scripts / 'helper.sh').write_text('''
check() {
  local std_prefix='--std='
  local std_dialect='gnu++17'
  c++ "${std_prefix}${std_dialect}" -c probe.cpp
}
check
''')
        expect_fail(run_checker(local_variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        declare_variable_workflow_standard_source = write_source(root / 'declare-variable-workflow-standard-flag-fail')
        declare_variable_workflow_root = declare_variable_workflow_standard_source.parent
        (declare_variable_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (declare_variable_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (declare_variable_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Build
        run: |
          declare std_prefix='--std='
          declare std_dialect='gnu++17'
          c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_fail(run_checker(declare_variable_workflow_standard_source), 'manual C++ standard flag in workflow/helper')

        accepted_then_manual_helper_source = write_source(root / 'accepted-then-manual-helper-standard-flag-fail')
        accepted_then_manual_helper_scripts = accepted_then_manual_helper_source.parent / '.github' / 'scripts'
        accepted_then_manual_helper_scripts.mkdir(parents=True)
        (accepted_then_manual_helper_scripts / 'helper.sh').write_text('c++ --std=gnu++20 -c probe.cpp; cmake -DCMAKE_CXX_STANDARD=17 -S source -B build\n')
        expect_fail(run_checker(accepted_then_manual_helper_source), 'manual C++ standard flag in workflow/helper')

        inline_variable_helper_standard_source = write_source(root / 'inline-variable-helper-standard-flag-fail')
        inline_variable_helper_scripts = inline_variable_helper_standard_source.parent / '.github' / 'scripts'
        inline_variable_helper_scripts.mkdir(parents=True)
        (inline_variable_helper_scripts / 'helper.sh').write_text('std_prefix=\'--std=\'; std_dialect=\'gnu++17\'; c++ "${std_prefix}${std_dialect}" -c probe.cpp\n')
        expect_fail(run_checker(inline_variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        local_inline_variable_helper_standard_source = write_source(root / 'local-inline-variable-helper-standard-flag-fail')
        local_inline_variable_helper_scripts = local_inline_variable_helper_standard_source.parent / '.github' / 'scripts'
        local_inline_variable_helper_scripts.mkdir(parents=True)
        (local_inline_variable_helper_scripts / 'helper.sh').write_text('''
check() {
  local std_prefix='--std=' std_dialect='gnu++17'
  c++ "${std_prefix}${std_dialect}" -c probe.cpp
}
check
''')
        expect_fail(run_checker(local_inline_variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        declare_inline_variable_workflow_standard_source = write_source(root / 'declare-inline-variable-workflow-standard-flag-fail')
        declare_inline_variable_workflow_root = declare_inline_variable_workflow_standard_source.parent
        (declare_inline_variable_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (declare_inline_variable_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (declare_inline_variable_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Build
        run: |
          declare std_prefix='--std=' std_dialect='gnu++17'
          c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_fail(run_checker(declare_inline_variable_workflow_standard_source), 'manual C++ standard flag in workflow/helper')

        export_inline_variable_helper_standard_source = write_source(root / 'export-inline-variable-helper-standard-flag-fail')
        export_inline_variable_helper_scripts = export_inline_variable_helper_standard_source.parent / '.github' / 'scripts'
        export_inline_variable_helper_scripts.mkdir(parents=True)
        (export_inline_variable_helper_scripts / 'helper.sh').write_text('''
export std_prefix='--std=' std_dialect='gnu++17'
c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_fail(run_checker(export_inline_variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        readonly_inline_variable_helper_standard_source = write_source(root / 'readonly-inline-variable-helper-standard-flag-fail')
        readonly_inline_variable_helper_scripts = readonly_inline_variable_helper_standard_source.parent / '.github' / 'scripts'
        readonly_inline_variable_helper_scripts.mkdir(parents=True)
        (readonly_inline_variable_helper_scripts / 'helper.sh').write_text('''
readonly std_prefix='--std=' std_dialect='gnu++17'
c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_fail(run_checker(readonly_inline_variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        chained_variable_helper_standard_source = write_source(root / 'chained-variable-helper-standard-flag-fail')
        chained_variable_helper_scripts = chained_variable_helper_standard_source.parent / '.github' / 'scripts'
        chained_variable_helper_scripts.mkdir(parents=True)
        (chained_variable_helper_scripts / 'helper.sh').write_text('std_prefix=\'--std=\' && std_dialect=\'gnu++17\' && c++ "${std_prefix}${std_dialect}" -c probe.cpp\n')
        expect_fail(run_checker(chained_variable_helper_standard_source), 'manual C++ standard flag in workflow/helper')

        declare_chained_variable_workflow_standard_source = write_source(root / 'declare-chained-variable-workflow-standard-flag-fail')
        declare_chained_variable_workflow_root = declare_chained_variable_workflow_standard_source.parent
        (declare_chained_variable_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (declare_chained_variable_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (declare_chained_variable_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Build
        run: |
          declare std_prefix='--std=' && declare std_dialect='gnu++17' && c++ "${std_prefix}${std_dialect}" -c probe.cpp
''')
        expect_fail(run_checker(declare_chained_variable_workflow_standard_source), 'manual C++ standard flag in workflow/helper')

        guardrail_workflow_source = write_source(root / 'guardrail-workflow-pass')
        guardrail_workflow_root = guardrail_workflow_source.parent
        (guardrail_workflow_root / '.github').mkdir(parents=True, exist_ok=True)
        (guardrail_workflow_root / '.github' / 'workflows').mkdir(parents=True, exist_ok=True)
        (guardrail_workflow_root / '.github' / 'workflows' / 'build.yml').write_text('''
name: Build
jobs:
  build:
    steps:
      - name: Check GNU++20 downgrade guardrail
        run: |
          ...
          check_cxx20_commands_clang build/cxx20-downgrade-guard \
            ...
            --forbidden-flag-substring=-std=c++17
''')
        expect_pass(run_checker(guardrail_workflow_source))

        helper_standard_source = write_source(root / 'helper-standard-flag-fail')
        helper_scripts = helper_standard_source.parent / '.github' / 'scripts'
        helper_scripts.mkdir(parents=True)
        (helper_scripts / 'helper.sh').write_text('cmake -DCMAKE_CXX_STANDARD=17 -S source -B build\n')
        expect_fail(run_checker(helper_standard_source), 'manual C++ standard flag in workflow/helper')

        other_workflow_source = write_source(root / 'other-workflow-standard-flag-fail')
        other_workflow_root = other_workflow_source.parent
        (other_workflow_root / '.github').mkdir(parents=True)
        (other_workflow_root / '.github' / 'workflows').mkdir(parents=True)
        (other_workflow_root / '.github' / 'workflows' / 'other.yml').write_text('''
name: Other
jobs:
  test:
    steps:
      - name: Build
        run: cmake -DCMAKE_CXX_STANDARD=17 -S source -B build
''')
        expect_fail(run_checker(other_workflow_source), 'manual C++ standard flag in workflow/helper')

        pgo_forbidden_flags_source = write_source(root / 'script-scan-pgo-forbidden-flags-pass')
        pgo_forbidden_scripts = root / 'script-scan-pgo-forbidden-flags-pass' / '.github' / 'scripts'
        pgo_forbidden_scripts.mkdir(parents=True)
        (pgo_forbidden_scripts / 'check_pgo_consume_chain.py').write_text("PGO_FORBIDDEN_CONSUME_FLAGS = ('/std:c++20',)\n")
        expect_pass(run_checker(pgo_forbidden_flags_source))
        ...
        split_string_concat_compile_flags_source = write_source(root / 'split-string-concat-compile-flags')
        split_string_concat_compile_flags_nested = split_string_concat_compile_flags_source / 'cmake'
        split_string_concat_compile_flags_nested.mkdir()
        (split_string_concat_compile_flags_nested / 'flags.cmake').write_text('string(CONCAT CMAKE_CXX_FLAGS "-std=" "gnu++17")\n')
        expect_fail(run_checker(split_string_concat_compile_flags_source), 'manual C++ standard flag in CMake')

        split_variable_string_concat_compile_flags_source = write_source(root / 'split-variable-string-concat-compile-flags')
        split_variable_string_concat_compile_flags_nested = split_variable_string_concat_compile_flags_source / 'cmake'
        split_variable_string_concat_compile_flags_nested.mkdir()
        (split_variable_string_concat_compile_flags_nested / 'flags.cmake').write_text('''
        set(STD_PREFIX "-std=")
        set(STD_DIALECT "gnu++17")
        string(CONCAT CMAKE_CXX_FLAGS ${STD_PREFIX} ${STD_DIALECT})
        ''')
        expect_fail(run_checker(split_variable_string_concat_compile_flags_source), 'manual C++ standard flag in CMake')

        split_variable_forward_standard_flag_source = write_source(root / 'split-variable-forward-standard-flag')
        split_variable_forward_standard_flag_nested = split_variable_forward_standard_flag_source / 'cmake'
        split_variable_forward_standard_flag_nested.mkdir()
        (split_variable_forward_standard_flag_nested / 'flags.cmake').write_text('''
        set(STD_PREFIX "-std=")
        set(STD_DIALECT "gnu++17")
        set(LOCAL_STD_FLAG "${STD_PREFIX}${STD_DIALECT}")
        add_compile_options(${LOCAL_STD_FLAG})
        ''')
        expect_fail(run_checker(split_variable_forward_standard_flag_source), 'manual C++ standard flag in CMake')

        split_list_append_compile_options_source = write_source(root / 'split-list-append-compile-options')
        split_list_append_compile_options_nested = split_list_append_compile_options_source / 'cmake'
        split_list_append_compile_options_nested.mkdir()
        (split_list_append_compile_options_nested / 'flags.cmake').write_text('list(APPEND X265_COMPILE_OPTIONS "-std=" "gnu++17")\n')
        expect_fail(run_checker(split_list_append_compile_options_source), 'manual C++ standard flag in CMake')

        split_docs_compile_flags_source = write_source(root / 'split-docs-compile-flags-pass')
        split_docs_compile_flags_nested = split_docs_compile_flags_source / 'cmake'
        split_docs_compile_flags_nested.mkdir()
        (split_docs_compile_flags_nested / 'docs.cmake').write_text('''
        set(STD_PREFIX_DOC "-std=")
        set(STD_DIALECT_DOC "gnu++17")
        string(CONCAT MY_CXX_FLAGS_DOC ${STD_PREFIX_DOC} ${STD_DIALECT_DOC} " appears in docs only")
        ''')
        expect_pass(run_checker(split_docs_compile_flags_source))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        runs:
          ...
          steps:
            - shell: bash
              run: c++ --std=gnu++20 --std=gnu++17 -c probe.cpp
        ''')
        expect_fail(run_checker(source), 'manual C++ standard flag in workflow/helper')
