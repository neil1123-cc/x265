...
def read_text(path):
    return _read_text_cached(*file_cache_key(path))
...
def _read_text_cached(path_str, _mtime_ns=None, _size=None):
...
def validate_yaml_structure(parsed, path, workflow_root):
    if not isinstance(parsed, dict):
        fail('YAML file did not parse to a mapping', path)
    if path.parent == workflow_root and 'jobs' not in parsed:
...
def validate_yaml_parse_with_pyyaml(repo_root, workflow_dir, action_dir):
    ...
    for path in yaml_files(repo_root, workflow_dir, action_dir):
        try:
            parsed = yaml_safe_load(read_text(path))
        except yaml.YAMLError as exc:
            line = getattr(getattr(exc, 'problem_mark', None), 'line', None)
            fail(str(exc), path, None if line is None else line + 1)
        validate_yaml_structure(parsed, path, workflow_root)
...
def load_yaml(repo_root, relative_path):
    ...
    try:
        ...
        try:
            parsed = _load_yaml_cached(*file_cache_key(path))
        except yaml.YAMLError as exc:
            line = getattr(getattr(exc, 'problem_mark', None), 'line', None)
            fail(str(exc), path, None if line is None else line + 1)
    if not isinstance(parsed, dict):
...
@lru_cache(maxsize=None)
def _load_yaml_cached(path_str, mtime_ns=None, size=None):
    return yaml_safe_load(_read_text_cached(path_str, mtime_ns, size))
...
def collect_run_blocks(path):
    return list(_collect_run_blocks_cached(*file_cache_key(path)))


@lru_cache(maxsize=None)
def _collect_run_blocks_cached(path_str, mtime_ns=None, size=None):
    path = Path(path_str)
    lines = _read_text_cached(path_str, mtime_ns, size).splitlines()
    blocks = []
    index = 0
    while index < len(lines):
...
def _python_script_command(repo_root, command):
    repo_root = Path(repo_root)
    if len(command) < 2:
        return None
    executable = Path(command[0]).name.lower()
    if executable not in {Path(sys.executable).name.lower(), 'python', 'python3'}:
        return None
    script = Path(command[1])
    if script.suffix != '.py':
        return None
    candidates = (script,) if script.is_absolute() else (script, repo_root / script)
...
def _run_python_script_in_process(repo_root, script, args):
    repo_root = Path(repo_root)
    output = StringIO()
    old_argv = sys.argv
    old_cwd = Path.cwd()
...
def run_guard(repo_root, *command):
    script = _python_script_command(repo_root, command)
    if script is not None:
        _run_python_script_in_process(repo_root, script, command[2:])
        return
...
def validate_python_file(repo_root, relative_path, missing_message, required_text=(), required_message='missing required python detail'):
    ...
    try:
        ...
        fail(f'python syntax check failed: {exc.msg}', path)
    text = read_text(path)
    for required in required_text:
        if required not in text:
            fail(f'{required_message}: {required}', path)
