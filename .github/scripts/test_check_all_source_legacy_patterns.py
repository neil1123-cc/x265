...
CHECKER = Path(__file__).with_name('check_all_source_legacy_patterns.py')
...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_targets(
            root,
            {
                ...
                'source/x265.h': '/* NULL in comment is fine */\nint ok = 0;\n',
    ...
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write_targets(root, {'source/foo.cpp': 'void* bad = NULL;\n'})
        expect_fail(run_checker(root), 'avoid legacy GNU++20-sensitive token in source tree: NULL')

    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write_targets(root, {'source/bar.h': 'void f() throw() {}\n'})
        expect_fail(run_checker(root), 'avoid legacy GNU++20-sensitive token in source tree: throw()')

    print('All source legacy pattern tests passed')
