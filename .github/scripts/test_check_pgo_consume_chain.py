...
def run_checker(metadata, profdata, build, *extra_args, require_dependency_fields=True):
    ...
    if require_dependency_fields:
        args.insert(-len(extra_args) if extra_args else len(args), '--require-dependency-fields')
...
def main():
    ...
    expect_metadata_failure(
        lambda metadata: metadata['dependencies'].__setitem__('ffmpeg_cache_key', 'ffmpeg-n8.1-full-v5-clang'),
        'profdata dependency cache suffix mismatch',
    )
    expect_metadata_failure(
        lambda metadata: metadata['dependencies'].__setitem__('obuparse_cache_key', 'obuparse-v2.0.2-clang-v2'),
        'profdata dependency cache suffix mismatch',
    )
    expect_metadata_failure(
        lambda metadata: metadata['dependencies'].__setitem__('lsmash_cache_key', 'lsmash-vimeo/l-smash-04e39f1fb232c332d4b04a1043c02c7c2d282d00-clang-coff-refptr-v3'),
        'profdata dependency cache suffix mismatch',
    )
    expect_metadata_failure(
        lambda metadata: metadata['dependencies'].__setitem__('gop_muxer_cache_key', 'gop-muxer-msg7086/gop_muxer-5677cf5ef905c2412ed31de300cd1a08b341d21d-lsmash-add-box-v3-clang-gnu20'),
        'profdata dependency cache suffix mismatch',
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        profdata_flag_path = '/tmp/x265.profdata'
        metadata, profdata, build = write_chain(root, f'-fprofile-instr-use={profdata_flag_path} -fprofile-sample-use=/tmp/sample.profdata')
        expect_fail(run_checker(metadata, profdata, build, f'--profdata-flag-path={profdata_flag_path}'), 'forbidden flag substring -fprofile-sample-use')

    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        profdata_flag_path = '/tmp/x265.profdata'
        metadata, profdata, build = write_chain(root, f'-fprofile-instr-use={profdata_flag_path} -fprofile-use=/tmp/gcc-pgo')
        expect_fail(run_checker(metadata, profdata, build, f'--profdata-flag-path={profdata_flag_path}'), 'forbidden flag substring -fprofile-use')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_compile_commands_records(build, [{
            'directory': str(build),
            'command': f'c++ /std:c++20 -fprofile-instr-use={profdata_flag_path} -c source/common/common.cpp',
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(metadata, profdata, build, f'--profdata-flag-path={profdata_flag_path}'), 'forbidden flag /std:c++20')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_compile_commands_records(build, [{
            'directory': str(build),
            'command': f'c++ -std=gnu++20 -fprofile-instr-use={profdata_flag_path} -c source/input/lavf.cpp',
            'arguments': ['c++', '-std=gnu++20', f'-fprofile-instr-use={profdata_flag_path}', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(metadata, profdata, build, f'--profdata-flag-path={profdata_flag_path}'), 'file field does not match compiled source source/input/lavf.cpp')
