...
def run_checker(build_dir, *args):
    return run_python_script_main(CHECKER, (str(build_dir), *args))
...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        mismatched_command_file_dir = root / 'mismatched-command-file'
        write_compile_commands_records(mismatched_command_file_dir, [{
            'directory': str(mismatched_command_file_dir),
            'command': 'c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/input/lavf.cpp',
            'file': str(root / 'source/output/mkv.cpp'),
        }])
        expect_fail(run_checker(mismatched_command_file_dir, '--required-file-substring=source/output/mkv.cpp', '--min-cpp-commands=1'), 'file field does not match compiled source source/input/lavf.cpp')

        mismatched_response_file_dir = root / 'mismatched-response-file'
        mismatched_response_file_dir.mkdir()
        (mismatched_response_file_dir / 'args.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -c source/input/lavf.cpp')
        write_compile_commands_records(mismatched_response_file_dir, [{
            'directory': str(mismatched_response_file_dir),
            'command': 'c++ @args.rsp',
            'file': str(root / 'source/output/mkv.cpp'),
        }])
        expect_fail(run_checker(mismatched_response_file_dir, '--required-file-substring=source/output/mkv.cpp', '--min-cpp-commands=1'), 'file field does not match compiled source source/input/lavf.cpp')

        mismatched_dual_field_file_dir = root / 'mismatched-dual-field-file'
        write_compile_commands_records(mismatched_dual_field_file_dir, [{
            'directory': str(mismatched_dual_field_file_dir),
            'command': 'c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/input/lavf.cpp',
            'arguments': ['c++', '-std=gnu++20', '-Wdeprecated', '-Werror=deprecated', '-c', 'source/output/mkv.cpp'],
            'file': str(root / 'source/output/mkv.cpp'),
        }])
        expect_fail(run_checker(mismatched_dual_field_file_dir, '--required-file-substring=source/output/mkv.cpp', '--min-cpp-commands=1'), 'file field does not match compiled source source/input/lavf.cpp')

        mismatched_dual_field_reverse_file_dir = root / 'mismatched-dual-field-reverse-file'
        write_compile_commands_records(mismatched_dual_field_reverse_file_dir, [{
            'directory': str(mismatched_dual_field_reverse_file_dir),
            'command': 'c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/output/mkv.cpp',
            'arguments': ['c++', '-std=gnu++20', '-Wdeprecated', '-Werror=deprecated', '-c', 'source/input/lavf.cpp'],
            'file': str(root / 'source/output/mkv.cpp'),
        }])
        expect_fail(run_checker(mismatched_dual_field_reverse_file_dir, '--required-file-substring=source/output/mkv.cpp', '--min-cpp-commands=1'), 'file field does not match compiled source source/input/lavf.cpp')

        mismatched_dual_field_response_file_dir = root / 'mismatched-dual-field-response-file'
        mismatched_dual_field_response_file_dir.mkdir()
        (mismatched_dual_field_response_file_dir / 'args.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -c source/input/lavf.cpp')
        write_compile_commands_records(mismatched_dual_field_response_file_dir, [{
            'directory': str(mismatched_dual_field_response_file_dir),
            'command': 'c++ @args.rsp -c source/output/mkv.cpp',
            'arguments': ['c++', '-std=gnu++20', '-Wdeprecated', '-Werror=deprecated', '-c', 'source/output/mkv.cpp'],
            'file': str(root / 'source/output/mkv.cpp'),
        }])
        expect_fail(run_checker(mismatched_dual_field_response_file_dir, '--required-file-substring=source/output/mkv.cpp', '--min-cpp-commands=1'), 'file field does not match compiled source source/input/lavf.cpp')
        ...
        ci_shape_reconplay_nested_arguments_response_missing_warning_dir = root / 'ci-shape-reconplay-nested-arguments-response-missing-warning'
        (ci_shape_reconplay_nested_arguments_response_missing_warning_dir / 'rsp').mkdir(parents=True)
        (ci_shape_reconplay_nested_arguments_response_missing_warning_dir / 'rsp' / 'common.rsp').write_text('-DX265_DEPTH=8')
        (ci_shape_reconplay_nested_arguments_response_missing_warning_dir / 'rsp' / 'reconplay.rsp').write_text('@common.rsp')
        write_compile_commands_records(ci_shape_reconplay_nested_arguments_response_missing_warning_dir, ci_shape_records(ci_shape_reconplay_nested_arguments_response_missing_warning_dir, root, {
            'source/output/reconplay.cpp': {
                'argument_flags': ['@rsp/reconplay.rsp'],
            },
        }))
        expect_fail(run_ci_shape_checker(ci_shape_reconplay_nested_arguments_response_missing_warning_dir), 'missing required flag -Werror=deprecated-volatile for file substring source/output/reconplay.cpp')
        ...
        clang_clang_wrapper_forbidden_flag_dir = root / 'clang-cl-clang-wrapper-forbidden-flag'
        write_compile_commands_records(clang_clang_wrapper_forbidden_flag_dir, [{
            'directory': str(clang_clang_wrapper_forbidden_flag_dir),
            'command': 'clang-cl /std:c++20 /clang:-w /c source/common/common.cpp',
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(clang_clang_wrapper_forbidden_flag_dir, '--forbidden-flag=-w', '--min-cpp-commands=1'), 'forbidden flag -w')

        clang_clang_wrapper_depth_dir = root / 'clang-cl-clang-wrapper-depth'
        write_compile_commands_records(clang_clang_wrapper_depth_dir, [{
            'directory': str(clang_clang_wrapper_depth_dir),
            'command': 'clang-cl /std:c++20 -DX265_DEPTH=8 /clang:-DX265_DEPTH=12 /c source/common/common.cpp',
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(clang_clang_wrapper_depth_dir, '--required-depth-define=-DX265_DEPTH=8', '--min-cpp-commands=1'), 'mixed depth defines -DX265_DEPTH=12')

        clang_clang_wrapper_split_define_depth_dir = root / 'clang-cl-clang-wrapper-split-define-depth'
        write_compile_commands_records(clang_clang_wrapper_split_define_depth_dir, [{
            'directory': str(clang_clang_wrapper_split_define_depth_dir),
            'arguments': ['clang-cl', '/TP', '/std:c++20', '/clang:-D', 'X265_DEPTH=12', '/c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(clang_clang_wrapper_split_define_depth_dir, '--required-depth-define=-DX265_DEPTH=8', '--min-cpp-commands=1'), 'missing -DX265_DEPTH=8')

        clang_clang_wrapper_forbidden_file_flag_dir = root / 'clang-cl-clang-wrapper-forbidden-file-flag'
        write_compile_commands_records(clang_clang_wrapper_forbidden_file_flag_dir, [{
            'directory': str(clang_clang_wrapper_forbidden_file_flag_dir),
            'command': 'clang-cl /std:c++20 /clang:-DENABLE_LAVF /c source/common/common.cpp',
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(clang_clang_wrapper_forbidden_file_flag_dir, '--forbidden-file-flag=source/common/common.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'), 'forbidden flag -DENABLE_LAVF for file substring source/common/common.cpp')

        clang_xclang_split_define_forbidden_file_flag_dir = root / 'clang-xclang-split-define-forbidden-file-flag'
        write_compile_commands_records(clang_xclang_split_define_forbidden_file_flag_dir, [{
            'directory': str(clang_xclang_split_define_forbidden_file_flag_dir),
            'arguments': ['clang-cl', '/TP', '/std:c++20', '-Xclang', '-D', '-Xclang', 'ENABLE_LAVF', '/c', 'source/encoder/encoder.cpp'],
            'file': str(root / 'source/encoder/encoder.cpp'),
        }])
        expect_fail(run_checker(clang_xclang_split_define_forbidden_file_flag_dir, '--forbidden-file-flag=source/encoder/encoder.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'), 'forbidden flag -DENABLE_LAVF for file substring source/encoder/encoder.cpp')

        clang_xclang_equals_split_define_depth_dir = root / 'clang-xclang-equals-split-define-depth'
        write_compile_commands_records(clang_xclang_equals_split_define_depth_dir, [{
            'directory': str(clang_xclang_equals_split_define_depth_dir),
            'arguments': ['clang-cl', '/TP', '/std:c++20', '-DX265_DEPTH=8', '-Xclang=-D', 'X265_DEPTH=12', '/c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(clang_xclang_equals_split_define_depth_dir, '--required-depth-define=-DX265_DEPTH=8', '--min-cpp-commands=1'), 'mixed depth defines -DX265_DEPTH=12')

        clang_clang_wrapper_std_drift_dir = root / 'clang-cl-clang-wrapper-std-drift'
        write_compile_commands_records(clang_clang_wrapper_std_drift_dir, [{
            'directory': str(clang_clang_wrapper_std_drift_dir),
            'command': 'clang-cl /std:c++20 /clang:-std=gnu++17 /c source/common/common.cpp',
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(clang_clang_wrapper_std_drift_dir), 'duplicate standard flags /std:c++20,-std=gnu++17')

        xclang_equals_forbidden_flag_dir = root / 'xclang-equals-forbidden-flag'
        write_compile_commands_records(xclang_equals_forbidden_flag_dir, [{
            'directory': str(xclang_equals_forbidden_flag_dir),
            'command': 'clang-cl /std:c++20 -Xclang=-w /c source/common/common.cpp',
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(xclang_equals_forbidden_flag_dir, '--forbidden-flag=-w', '--min-cpp-commands=1'), 'forbidden flag -w')
        ...
        msvc_uppercase_std_dir = root / 'pass-msvc-uppercase-std'
        write_compile_commands_records(msvc_uppercase_std_dir, [{
            'directory': str(msvc_uppercase_std_dir),
            'command': 'clang-cl /TP /STD:C++20 /c source/common/template.inc',
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_pass(run_checker(msvc_uppercase_std_dir, '--min-cpp-commands=1'))

        msvc_mixedcase_old_std_dir = root / 'msvc-mixedcase-old-std'
        write_compile_commands_records(msvc_mixedcase_old_std_dir, [{
            'directory': str(msvc_mixedcase_old_std_dir),
            'arguments': ['clang-cl', '/TP', '/Std:C++17', '/c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(msvc_mixedcase_old_std_dir), 'old standard flag /std:c++17')
        ...
        msvc_required_depth_missing_dir = root / 'msvc-required-depth-missing'
        ...
        expect_pass(run_checker(msvc_response_msvc_define_depth_missing_dir, '--required-depth-define=-DX265_DEPTH=8', '--min-cpp-commands=1'))

        msvc_response_msvc_define_depth_mixed_dir = root / 'msvc-response-msvc-define-depth-mixed'
        msvc_response_msvc_define_depth_mixed_dir.mkdir()
        (msvc_response_msvc_define_depth_mixed_dir / 'lang.rsp').write_text('/TP /std:c++20 /DX265_DEPTH=8 /DX265_DEPTH=12')
        write_compile_commands_records(msvc_response_msvc_define_depth_mixed_dir, [{
            'directory': str(msvc_response_msvc_define_depth_mixed_dir),
            'command': 'clang-cl @lang.rsp /c source/common/template.inc',
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(msvc_response_msvc_define_depth_mixed_dir, '--required-depth-define=-DX265_DEPTH=8', '--min-cpp-commands=1'), 'mixed depth defines -DX265_DEPTH=12')

        msvc_response_wrapper_forbidden_flag_dir = root / 'msvc-response-wrapper-forbidden-flag'
        msvc_response_wrapper_forbidden_flag_dir.mkdir()
        (msvc_response_wrapper_forbidden_flag_dir / 'lang.rsp').write_text('/TP /std:c++20 /clang:-w')
        write_compile_commands_records(msvc_response_wrapper_forbidden_flag_dir, [{
            'directory': str(msvc_response_wrapper_forbidden_flag_dir),
            'command': 'clang-cl @lang.rsp /c source/common/template.inc',
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(msvc_response_wrapper_forbidden_flag_dir, '--forbidden-flag=-w', '--min-cpp-commands=1'), 'forbidden flag -w')
        ...
        command_c_arguments_cxx_boundary_dir = root / 'command-c-arguments-cxx-boundary'
        write_compile_commands_records(command_c_arguments_cxx_boundary_dir, [{
            'directory': str(command_c_arguments_cxx_boundary_dir),
            'command': 'cc -std=c11 -c source/common/template.inc',
            'arguments': ['cc', '-x', 'c++', '-std=gnu++20', '-Wdeprecated', '-Werror=deprecated', '-c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(command_c_arguments_cxx_boundary_dir, '--min-cpp-commands=1'), 'duplicate standard flags -std=gnu++20,-std=c11')

        command_cxx_arguments_c_boundary_dir = root / 'command-cxx-arguments-c-boundary'
        write_compile_commands_records(command_cxx_arguments_c_boundary_dir, [{
            'directory': str(command_cxx_arguments_c_boundary_dir),
            'command': 'cc -x c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/common/template.inc',
            'arguments': ['cc', '-std=c11', '-c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(command_cxx_arguments_c_boundary_dir, '--min-cpp-commands=1'), 'duplicate standard flags -std=c11,-std=gnu++20')
        ...
        msvc_tp_old_std_dir = root / 'msvc-tp-cxx-language-old-std'
        write_compile_commands_records(msvc_tp_old_std_dir, [{
            'directory': str(msvc_tp_old_std_dir),
            'command': 'clang-cl /TP /std:c++17 /c source/common/template.inc',
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(msvc_tp_old_std_dir), 'old standard flag /std:c++17')

        msvc_fused_tp_old_std_dir = root / 'msvc-fused-tp-cxx-language-old-std'
        write_compile_commands_records(msvc_fused_tp_old_std_dir, [{
            'directory': str(msvc_fused_tp_old_std_dir),
            'arguments': ['clang-cl', '/Tpsource/common/template.inc', '/std:c++17', '/c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(msvc_fused_tp_old_std_dir), 'old standard flag /std:c++17')

        msvc_tc_language_dir = root / 'msvc-tc-c-language'
        write_compile_commands_records(msvc_tc_language_dir, [{
            'directory': str(msvc_tc_language_dir),
            'arguments': ['clang-cl', '/TC', '/std:c++20', '/c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(msvc_tc_language_dir), 'no C++ compile commands')
        ...
        extensionless_response_forbidden_flag_dir = root / 'extensionless-response-file-forbidden-exact-flag'
        extensionless_response_forbidden_flag_dir.mkdir()
        (extensionless_response_forbidden_flag_dir / 'args').write_text('-DX265_DEPTH=12')
        write_compile_commands(
            extensionless_response_forbidden_flag_dir,
            'c++ -std=gnu++20 -Wdeprecated -Werror=deprecated @args -c source/common/common.cpp',
        )
        expect_fail(run_checker(extensionless_response_forbidden_flag_dir, '--forbidden-flag=-DX265_DEPTH=12'), 'forbidden flag -DX265_DEPTH=12')
        ...
        quoted_windows_path_source_substring_response_dir = root / 'quoted-windows-path-source-substring-response-file'
        quoted_windows_path_source_substring_response_dir.mkdir()
        (quoted_windows_path_source_substring_response_dir / 'args file.rsp').write_text('-std=gnu++20 -DENABLE_LAVF "-FI=C:/Program Files/x265/config.h"')
        write_compile_commands_records(quoted_windows_path_source_substring_response_dir, [{
            'directory': str(quoted_windows_path_source_substring_response_dir),
            'arguments': ['c++', '@args file.rsp', '-c', 'source\\input\\lavf.cpp'],
            'file': str(root / 'source\\input\\lavf.cpp'),
        }])
        ...
        mixed_case_response_suffix_required_file_flag_dir = root / 'mixed-case-response-suffix-required-file-flag'
        mixed_case_response_suffix_required_file_flag_dir.mkdir()
        (mixed_case_response_suffix_required_file_flag_dir / 'ARGS.ReSpOnSe').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DENABLE_LAVF')
        write_compile_commands_records(mixed_case_response_suffix_required_file_flag_dir, [{
            'directory': str(mixed_case_response_suffix_required_file_flag_dir),
            'arguments': ['c++', '@ARGS.ReSpOnSe', '-c', 'source/input/lavf.cpp'],
            'file': str(root / 'source/input/lavf.cpp'),
        }])
        expect_pass(run_checker(mixed_case_response_suffix_required_file_flag_dir, '--required-file-flag=source/input/lavf.cpp=-DENABLE_LAVF'))
        ...
        response_cycle_required_file_flag_dir = root / 'response-file-cycle-required-file-flag'
        response_cycle_required_file_flag_dir.mkdir()
        (response_cycle_required_file_flag_dir / 'args.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DENABLE_LAVF @args.rsp')
        write_compile_commands(response_cycle_required_file_flag_dir, 'c++ @args.rsp -c source/input/lavf.cpp', 'source/input/lavf.cpp')
        expect_pass(run_checker(response_cycle_required_file_flag_dir, '--required-file-flag=source/input/lavf.cpp=-DENABLE_LAVF'))

        response_cycle_required_prefix_dir = root / 'response-file-cycle-required-prefix'
        response_cycle_required_prefix_dir.mkdir()
        (response_cycle_required_prefix_dir / 'args.rsp').write_text('-std=gnu++20 -fprofile-instr-use=/tmp/x265.profdata @args.rsp')
        write_compile_commands(response_cycle_required_prefix_dir, 'c++ @args.rsp -c source/common/common.cpp')
        expect_pass(run_checker(response_cycle_required_prefix_dir, '--required-flag-prefix=-fprofile-instr-use='))

        response_cycle_arguments_missing_required_prefix_dir = root / 'response-file-cycle-arguments-missing-required-prefix'
        response_cycle_arguments_missing_required_prefix_dir.mkdir()
        (response_cycle_arguments_missing_required_prefix_dir / 'args.rsp').write_text('-std=gnu++20 -fprofile-sample-use=/tmp/x265.profdata @args.rsp')
        write_compile_commands_records(response_cycle_arguments_missing_required_prefix_dir, [{
            'directory': str(response_cycle_arguments_missing_required_prefix_dir),
            'arguments': ['c++', '@args.rsp', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(response_cycle_arguments_missing_required_prefix_dir, '--required-flag-prefix=-fprofile-instr-use='), 'missing required flag prefix -fprofile-instr-use=')

        response_dual_field_std_drift_dir = root / 'response-dual-field-required-file-flag-std-drift'
        response_dual_field_std_drift_dir.mkdir()
        (response_dual_field_std_drift_dir / 'command.rsp').write_text('-std=gnu++20 -DENABLE_LAVF')
        write_compile_commands_records(response_dual_field_std_drift_dir, [{
            'directory': str(response_dual_field_std_drift_dir),
            'command': 'c++ @command.rsp -c source/input/lavf.cpp',
            'arguments': ['c++', '-std=c++20', '-DENABLE_LAVF', '-c', 'source/input/lavf.cpp'],
            'file': str(root / 'source/input/lavf.cpp'),
        }])
        ...
        (response_dual_field_required_prefix_missing_dir / 'command.rsp').write_text('-std=gnu++20')
        (response_dual_field_required_prefix_missing_dir / 'arguments.rsp').write_text('-std=gnu++20 -fprofile-instr-use=/tmp/x265.profdata')
        write_compile_commands_records(response_dual_field_required_prefix_missing_dir, [{
            'directory': str(response_dual_field_required_prefix_missing_dir),
            'command': 'c++ @command.rsp -c source/common/common.cpp',
            'arguments': ['c++', '@arguments.rsp', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        ...
        response_dual_field_forbidden_substring_split_dir = root / 'response-dual-field-forbidden-substring-split'
        response_dual_field_forbidden_substring_split_dir.mkdir()
        (response_dual_field_forbidden_substring_split_dir / 'command.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -Wno-error=deprecated-declarations')
        (response_dual_field_forbidden_substring_split_dir / 'arguments.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated')
        write_compile_commands_records(response_dual_field_forbidden_substring_split_dir, [{
            'directory': str(response_dual_field_forbidden_substring_split_dir),
            'command': 'c++ @command.rsp -c source/common/common.cpp',
            'arguments': ['c++', '@arguments.rsp', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(response_dual_field_forbidden_substring_split_dir, '--forbidden-flag-substring=-Wno-error=deprecated'), 'forbidden flag substring -Wno-error=deprecated')
        ...
        response_arguments_forbidden_flag_dir = root / 'response-arguments-forbidden-flag-dual-field'
        response_arguments_forbidden_flag_dir.mkdir()
        (response_arguments_forbidden_flag_dir / 'args.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DX265_DEPTH=12')
        write_compile_commands_records(response_arguments_forbidden_flag_dir, [{
            'directory': str(response_arguments_forbidden_flag_dir),
            'command': 'c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/common/common.cpp',
            'arguments': ['c++', '@args.rsp', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(response_arguments_forbidden_flag_dir, '--forbidden-flag=-DX265_DEPTH=12'), 'forbidden flag -DX265_DEPTH=12')
        ...
        msvc_define_alias_required_file_flag_dir = root / 'msvc-define-alias-required-file-flag'
        msvc_define_alias_required_file_flag_dir.mkdir()
        (msvc_define_alias_required_file_flag_dir / 'lang.rsp').write_text('/TP /std:c++20 /DENABLE_LAVF')
        write_compile_commands_records(msvc_define_alias_required_file_flag_dir, [{
            'directory': str(msvc_define_alias_required_file_flag_dir),
            'command': 'clang-cl @lang.rsp /c source/input/lavf.cpp',
            'file': str(root / 'source/input/lavf.cpp'),
        }])
        expect_pass(run_checker(msvc_define_alias_required_file_flag_dir, '--required-file-flag=source/input/lavf.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'))

        msvc_lowercase_define_alias_required_file_flag_dir = root / 'msvc-lowercase-define-alias-required-file-flag'
        msvc_lowercase_define_alias_required_file_flag_dir.mkdir()
        (msvc_lowercase_define_alias_required_file_flag_dir / 'lang.rsp').write_text('/TP /std:c++20 /dENABLE_LAVF')
        write_compile_commands_records(msvc_lowercase_define_alias_required_file_flag_dir, [{
            'directory': str(msvc_lowercase_define_alias_required_file_flag_dir),
            'command': 'clang-cl @lang.rsp /c source/input/lavf.cpp',
            'file': str(root / 'source/input/lavf.cpp'),
        }])
        expect_pass(run_checker(msvc_lowercase_define_alias_required_file_flag_dir, '--required-file-flag=source/input/lavf.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'))

        msvc_define_alias_forbidden_file_flag_dir = root / 'msvc-define-alias-forbidden-file-flag'
        msvc_define_alias_forbidden_file_flag_dir.mkdir()
        (msvc_define_alias_forbidden_file_flag_dir / 'lang.rsp').write_text('/TP /std:c++20 /DENABLE_LAVF')
        write_compile_commands_records(msvc_define_alias_forbidden_file_flag_dir, [{
            'directory': str(msvc_define_alias_forbidden_file_flag_dir),
            'command': 'clang-cl @lang.rsp /c source/encoder/encoder.cpp',
            'file': str(root / 'source/encoder/encoder.cpp'),
        }])
        expect_fail(run_checker(msvc_define_alias_forbidden_file_flag_dir, '--forbidden-file-flag=source/encoder/encoder.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'), 'forbidden flag -DENABLE_LAVF for file substring source/encoder/encoder.cpp')

        msvc_lowercase_define_alias_forbidden_file_flag_dir = root / 'msvc-lowercase-define-alias-forbidden-file-flag'
        msvc_lowercase_define_alias_forbidden_file_flag_dir.mkdir()
        (msvc_lowercase_define_alias_forbidden_file_flag_dir / 'lang.rsp').write_text('/TP /std:c++20 /dENABLE_LAVF')
        write_compile_commands_records(msvc_lowercase_define_alias_forbidden_file_flag_dir, [{
            'directory': str(msvc_lowercase_define_alias_forbidden_file_flag_dir),
            'command': 'clang-cl @lang.rsp /c source/encoder/encoder.cpp',
            'file': str(root / 'source/encoder/encoder.cpp'),
        }])
        expect_fail(run_checker(msvc_lowercase_define_alias_forbidden_file_flag_dir, '--forbidden-file-flag=source/encoder/encoder.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'), 'forbidden flag -DENABLE_LAVF for file substring source/encoder/encoder.cpp')

        split_define_mixed_depth_dir = root / 'split-define-mixed-depth'
        write_compile_commands_records(split_define_mixed_depth_dir, [{
            'directory': str(split_define_mixed_depth_dir),
            'arguments': ['c++', '-std=gnu++20', '-DX265_DEPTH=8', '-D', 'X265_DEPTH=12', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_fail(run_checker(split_define_mixed_depth_dir, '--required-depth-define=-DX265_DEPTH=8', '--min-cpp-commands=1'), 'mixed depth defines -DX265_DEPTH=12')

        clang_wrapped_define_forbidden_file_flag_dir = root / 'clang-wrapped-define-forbidden-file-flag'
        write_compile_commands_records(clang_wrapped_define_forbidden_file_flag_dir, [{
            'directory': str(clang_wrapped_define_forbidden_file_flag_dir),
            'command': 'clang-cl /TP /std:c++20 /clang:/DENABLE_LAVF /c source/encoder/encoder.cpp',
            'file': str(root / 'source/encoder/encoder.cpp'),
        }])
        expect_fail(run_checker(clang_wrapped_define_forbidden_file_flag_dir, '--forbidden-file-flag=source/encoder/encoder.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'), 'forbidden flag -DENABLE_LAVF for file substring source/encoder/encoder.cpp')

        clang_wrapped_lowercase_define_forbidden_file_flag_dir = root / 'clang-wrapped-lowercase-define-forbidden-file-flag'
        write_compile_commands_records(clang_wrapped_lowercase_define_forbidden_file_flag_dir, [{
            'directory': str(clang_wrapped_lowercase_define_forbidden_file_flag_dir),
            'command': 'clang-cl /TP /std:c++20 /clang:/dENABLE_LAVF /c source/encoder/encoder.cpp',
            'file': str(root / 'source/encoder/encoder.cpp'),
        }])
        expect_fail(run_checker(clang_wrapped_lowercase_define_forbidden_file_flag_dir, '--forbidden-file-flag=source/encoder/encoder.cpp=-DENABLE_LAVF', '--min-cpp-commands=1'), 'forbidden flag -DENABLE_LAVF for file substring source/encoder/encoder.cpp')
        ...
        write_compile_commands_records(msvc_tp_response_forbidden_min_cpp_mixed_language_dir, [{
            'directory': str(msvc_tp_response_forbidden_min_cpp_mixed_language_dir),
            'command': 'clang-cl @tp.rsp /c source/common/template.inc',
            'file': str(root / 'source/common/template.inc'),
        }, {
            'directory': str(msvc_tp_response_forbidden_min_cpp_mixed_language_dir),
            'arguments': ['cc', '-x', 'c++', '-std=gnu++20', '-c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        ...
        write_compile_commands_records(response_language_flag_dual_field_suffix_dir, [{
            ...
            'command': 'cc @lang.rsp -c source/common/common.cpp',
            'arguments': ['c++', '-std=gnu++20', '-Wdeprecated', '-Werror=deprecated', '-c', 'source/common/common.cpp'],
            'file': str(root / 'source/common/common.cpp'),
        }])
        expect_pass(run_checker(response_language_flag_dual_field_suffix_dir, '--required-flag=-Werror=deprecated', '--min-cpp-commands=1'))

        response_dual_field_command_c_arguments_cxx_boundary_dir = root / 'response-dual-field-command-c-arguments-cxx-boundary'
        (response_dual_field_command_c_arguments_cxx_boundary_dir / 'rsp').mkdir(parents=True)
        (response_dual_field_command_c_arguments_cxx_boundary_dir / 'rsp' / 'command.rsp').write_text('-std=c11')
        (response_dual_field_command_c_arguments_cxx_boundary_dir / 'rsp' / 'arguments.rsp').write_text('-x c++ -std=gnu++20 -Wdeprecated -Werror=deprecated')
        write_compile_commands_records(response_dual_field_command_c_arguments_cxx_boundary_dir, [{
            'directory': str(response_dual_field_command_c_arguments_cxx_boundary_dir),
            'command': 'cc @rsp/command.rsp -c source/common/template.inc',
            'arguments': ['cc', '@rsp/arguments.rsp', '-c', 'source/common/template.inc'],
            'file': str(root / 'source/common/template.inc'),
        }])
        expect_fail(run_checker(response_dual_field_command_c_arguments_cxx_boundary_dir, '--min-cpp-commands=1'), 'duplicate standard flags -std=gnu++20,-std=c11')

        windows_backslash_response_required_file_substring_dir = root / 'windows-backslash-response-required-file-substring'
        (windows_backslash_response_required_file_substring_dir / 'rsp').mkdir(parents=True)
        (windows_backslash_response_required_file_substring_dir / 'rsp' / 'lavf.rsp').write_text('-std=gnu++20 -DENABLE_LAVF')
        write_compile_commands_records(windows_backslash_response_required_file_substring_dir, [{
            'directory': str(windows_backslash_response_required_file_substring_dir),
            'command': 'c++ @"rsp\\lavf.rsp" -c source\\input\\lavf.cpp',
            'file': str(root / 'source\\input\\lavf.cpp'),
        }])
        expect_pass(run_checker(windows_backslash_response_required_file_substring_dir, '--required-file-substring=source/input/lavf.cpp', '--required-file-flag=source/input/lavf.cpp=-DENABLE_LAVF'))

        windows_backslash_response_forbidden_file_substring_dir = root / 'windows-backslash-response-forbidden-file-substring'
        (windows_backslash_response_forbidden_file_substring_dir / 'rsp').mkdir(parents=True)
        (windows_backslash_response_forbidden_file_substring_dir / 'rsp' / 'mp4.rsp').write_text('-std=gnu++20 -DENABLE_LAVF')
        write_compile_commands_records(windows_backslash_response_forbidden_file_substring_dir, [{
            'directory': str(windows_backslash_response_forbidden_file_substring_dir),
            'arguments': ['c++', '@"rsp\\mp4.rsp"', '-c', 'source\\output\\mp4.cpp'],
            'file': str(root / 'source\\output\\mp4.cpp'),
        }])
        expect_fail(run_checker(windows_backslash_response_forbidden_file_substring_dir, '--forbidden-file-substring=source/output/mp4.cpp'), 'forbidden compile command for file substring source/output/mp4.cpp')

        windows_response_depth_exclude_uppercase_suffix_dir = root / 'windows-response-depth-exclude-uppercase-suffix'
        (windows_response_depth_exclude_uppercase_suffix_dir / 'rsp').mkdir(parents=True)
        (windows_response_depth_exclude_uppercase_suffix_dir / 'rsp' / 'excluded-command.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DX265_DEPTH=12')
        (windows_response_depth_exclude_uppercase_suffix_dir / 'rsp' / 'excluded-arguments.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DX265_DEPTH=8 -DX265_DEPTH=10')
        (windows_response_depth_exclude_uppercase_suffix_dir / 'rsp' / 'common-command.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DX265_DEPTH=8')
        (windows_response_depth_exclude_uppercase_suffix_dir / 'rsp' / 'common-arguments.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated -DX265_DEPTH=8')
        write_compile_commands_records(windows_response_depth_exclude_uppercase_suffix_dir, [{
            'directory': str(windows_response_depth_exclude_uppercase_suffix_dir),
            'command': 'c++ @"rsp\\excluded-command.rsp" -c source\\dynamicHDR10\\JSON11.CPP',
            'arguments': ['c++', '@"rsp\\excluded-arguments.rsp"', '-c', 'source\\dynamicHDR10\\JSON11.CPP'],
            'file': str(root / 'source\\dynamicHDR10\\JSON11.CPP'),
        }, {
        ...
        expect_pass(run_checker(windows_response_depth_exclude_uppercase_suffix_dir, '--required-depth-define=-DX265_DEPTH=8', '--depth-exclude-path=source/dynamicHDR10/'))

        msvc_fused_tp_quoted_response_command_dir = root / 'msvc-fused-tp-quoted-response-command'
        msvc_fused_tp_quoted_response_command_dir.mkdir()
        (msvc_fused_tp_quoted_response_command_dir / 'lang file.rsp').write_text('/Tpsource/common/template.inc /std:c++20')
        write_compile_commands_records(msvc_fused_tp_quoted_response_command_dir, [{
            'directory': str(msvc_fused_tp_quoted_response_command_dir),
        ...
        write_compile_commands_records(duplicate_source_non_cpp_dir, [
            {
                'directory': str(duplicate_source_non_cpp_dir),
                'command': 'c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/common/common.cpp',
                'file': str(root / 'source/common/common.cpp'),
            },
            {
        ...
        expect_fail(run_checker(duplicate_source_non_cpp_dir, '--min-cpp-commands=2'), 'expected at least 2 unique C++ compile commands')

        duplicate_source_response_dir = root / 'duplicate-source-min-cpp-commands-response-file'
        duplicate_source_response_dir.mkdir()
        (duplicate_source_response_dir / 'std.rsp').write_text('-std=gnu++20 -Wdeprecated -Werror=deprecated')
        write_compile_commands_records(duplicate_source_response_dir, [
            {
                'directory': str(duplicate_source_response_dir),
                'command': 'c++ @std.rsp -c source/common/common.cpp',
                'file': str(root / 'source/common/common.cpp'),
            },
            {
                'directory': str(duplicate_source_response_dir),
                'command': 'c++ @std.rsp -DSECOND_PASS=1 -c source/common/common.cpp',
                'file': str(root / 'source/common/common.cpp'),
            },
        ])
        ...
        write_compile_commands_entries(duplicate_source_forbidden_flag_dir, [
            ('c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/common/common.cpp', 'source/common/common.cpp'),
            ('c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -DX265_DEPTH=12 -c source/common/common.cpp', 'source/common/common.cpp'),
            ('c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/encoder/encoder.cpp', 'source/encoder/encoder.cpp'),
        ])
        expect_fail(run_checker(duplicate_source_forbidden_flag_dir, '--forbidden-flag=-DX265_DEPTH=12', '--min-cpp-commands=2'), 'forbidden flag -DX265_DEPTH=12')

        duplicate_source_forbidden_flag_substring_dir = root / 'duplicate-source-forbidden-flag-substring'
        write_compile_commands_entries(duplicate_source_forbidden_flag_substring_dir, [
            ('c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/common/common.cpp', 'source/common/common.cpp'),
            ('c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -Wno-deprecated -c source/common/common.cpp', 'source/common/common.cpp'),
            ('c++ -std=gnu++20 -Wdeprecated -Werror=deprecated -c source/encoder/encoder.cpp', 'source/encoder/encoder.cpp'),
        ])
        expect_fail(run_checker(duplicate_source_forbidden_flag_substring_dir, '--forbidden-flag-substring=-Wno-deprecated', '--min-cpp-commands=2'), 'forbidden flag substring -Wno-deprecated')
