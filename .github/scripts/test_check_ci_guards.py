...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cases = (
            ...
            case(lambda repo: replace_text(windows_deps_action(repo), '        sparse-checkout: |\n          gop_muxer.cpp\n          gop_muxer.h', '        sparse-checkout: .'), 'Checkout GOP muxer must sparse-checkout only gop_muxer.cpp and gop_muxer.h', 'windows-deps-checkout-scope'),
            ...
            case(
                ...
                'C++20 warning scan dependency setup must pin ffmpeg-cache-suffix=lavf-v4-clang',
                'warning-scan-dependencies',
            ),
            case(
                lambda repo: replace_text(
            ...
            case(lambda repo: replace_text(build_workflow(repo), 'build/cxx20-linux-gcc-compile-commands/x265 --input build/cxx20-linux-gcc-compile-commands/smoke_linux_gcc.yuv --input-res 96x96', 'build/cxx20-linux-gcc-compile-commands/x265 --input build/cxx20-linux-gcc-compile-commands/smoke_linux_gcc.yuv --input-res 128x128'), 'Linux GCC smoke --input-res must be 96x96, got 128x128', 'linux-gcc-smoke'),
            ...
            case(
                lambda repo: replace_text(
                    ...
                    'ffmpeg -hide_banner -loglevel error -y -f lavfi -i testsrc2=size=128x72:rate=24 -frames:v 4 -pix_fmt yuv420p build/cxx20-warning-scan-shared-deps-asm/smoke_shared_deps_seed.mp4',
                ),
            ...
            case(
                lambda repo: replace_text(
                    ...
                    'build/cxx20-warning-scan-shared-deps-asm/x265.exe build/cxx20-warning-scan-shared-deps-asm/smoke_shared_deps_seed.mp4 --frames 4 --bframes 0 --keyint 1 --min-keyint 1 --no-progress -o build/cxx20-warning-scan-shared-deps-asm/smoke_shared_deps_out.hevc',
                ),
            ...
            case(lambda repo: replace_text(repo / '.github' / 'scripts' / 'build_x265_profiling.sh', 'resolve_cpu_march_flag()', 'resolve_cpu_march_flag_disabled()', count=1), 'build profiling helper missing detail: resolve_cpu_march_flag()', 'required-snippets'),
            case(lambda repo: replace_text(repo / '.github' / 'scripts' / 'build_x265_profiling.sh', 'profiling_cxx_flags_with_cpu="${profiling_cxx_flags_with_cpu} ${target_cpu_cxx_flag}"', 'profiling_cxx_flags_with_cpu="$profiling_cxx_flags"', count=1), 'build profiling helper missing detail: profiling_cxx_flags_with_cpu="${profiling_cxx_flags_with_cpu} ${target_cpu_cxx_flag}"', 'required-snippets'),
            case(lambda repo: replace_text(repo / '.github' / 'scripts' / 'build_x265_profiling.sh', 'profiling_check_args+=(--required-flag="$target_cpu_cxx_flag")', 'profiling_check_args=()', count=1), 'build profiling helper missing detail: profiling_check_args+=(--required-flag="$target_cpu_cxx_flag")', 'required-snippets'),
            case(
                lambda repo: replace_text(
                    profiling_workflow(repo),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), '      profile_target:', "      target_cpu:\n        description: Target CPU branch to publish profdata for\n        required: false\n        default: x86-64\n        type: choice\n        options:\n          - x86-64\n          - haswell\n      profile_target:", count=1), 'Build PGO workflow_dispatch must not expose target_cpu input; generated PGO profdata is x86-64 baseline only', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), "  group: ${{ github.workflow }}-${{ github.ref }}-x86-64-${{ inputs.profile_target || 'all' }}", '  group: ${{ github.workflow }}-${{ github.ref }}'), 'Build PGO concurrency group must serialize by ref, x86-64 baseline, and profile_target', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), "        target-cpu: x86-64", "        target-cpu: ${{ inputs.target_cpu || 'x86-64' }}", count=1), 'Build PGO profiling action must set target-cpu=x86-64', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), '        target_cpu=x86-64', '        target_cpu="${{ github.event.inputs.target_cpu || \'x86-64\' }}"', count=1), 'missing required Build PGO workflow guard snippet: target_cpu=x86-64', 'required-snippets'),
            case(lambda repo: replace_text(build_pgo_workflow(repo), 'profdata_branch="profdata-x86-64-${profile_target}"', 'profdata_branch="profdata-${target_cpu}-${profile_target}"'), 'Build PGO profdata publish must publish to x86-64 baseline profdata branches', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), '          "target_cpu": "${target_cpu}",\n', '', count=1), 'Build PGO profdata metadata must record the target CPU', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), '          --expected-cpu="$target_cpu" \\\n', '', count=1), 'missing required Build PGO workflow guard snippet: --expected-cpu="$target_cpu"', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_pgo_workflow(repo), '          --require-target-cpu \\\n', '', count=1), 'missing required Build PGO workflow guard snippet: --require-target-cpu', 'required-snippets'),
            ...
            case(lambda repo: replace_text(build_workflow(repo), '          if [ "${CI_FULL_EVENT}" = \'true\' ]; then', '          if true; then', count=2), 'Build workflow Fetch PGO Profdata must include: if [ "${CI_FULL_EVENT}" = \'true\' ]; then', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), 'fetch_output=$(timeout 180s git -c http.lowSpeedLimit=1024 -c http.lowSpeedTime=30 fetch --quiet origin "$branch" --depth=1 2>&1)', 'fetch_output=$(git ls-remote --exit-code --heads origin "$branch" 2>&1)'), 'Build workflow Fetch PGO Profdata must include: fetch_output=$(timeout 180s git -c http.lowSpeedLimit=1024 -c http.lowSpeedTime=30 fetch --quiet origin "$branch" --depth=1 2>&1)', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), 'fetch_output=$(timeout 180s git -c http.lowSpeedLimit=1024 -c http.lowSpeedTime=30 fetch --quiet origin "$branch" --depth=1 2>&1)', 'git ls-remote --exit-code --heads origin "$branch" >/dev/null\n              fetch_output=$(timeout 180s git -c http.lowSpeedLimit=1024 -c http.lowSpeedTime=30 fetch --quiet origin "$branch" --depth=1 2>&1)'), 'Build workflow Fetch PGO Profdata must not preflight branches with git ls-remote before git fetch', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '                  echo "::warning::PGO profdata fetch timed out: target=${target} branch=${branch} after 180s"', '                  echo "PGO profdata fetch timed out"'), 'Build workflow Fetch PGO Profdata must include: echo "::warning::PGO profdata fetch timed out: target=${target} branch=${branch} after 180s"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '                  echo "PGO profdata fetch timed out: target=${target} branch=${branch} after 180s"', '                  echo "::warning::PGO profdata fetch timed out: target=${target} branch=${branch} after 180s"', count=1), 'Build workflow Fetch PGO Profdata must include: echo "PGO profdata fetch timed out: target=${target} branch=${branch} after 180s"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '                  echo "::warning::PGO profdata fetch failed: target=${target} branch=${branch} status=${fetch_status}"', '                  echo "PGO profdata fetch failed"'), 'Build workflow Fetch PGO Profdata must include: echo "::warning::PGO profdata fetch failed: target=${target} branch=${branch} status=${fetch_status}"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '                  echo "PGO profdata fetch failed: target=${target} branch=${branch} status=${fetch_status}"', '                  echo "::warning::PGO profdata fetch failed: target=${target} branch=${branch} status=${fetch_status}"', count=1), 'Build workflow Fetch PGO Profdata must include: echo "PGO profdata fetch failed: target=${target} branch=${branch} status=${fetch_status}"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '                  printf \'%s\\n\' "$metadata_check_output" | sed \'s/^::warning::/PGO fallback probe warning: /\'', '                  printf \'%s\\n\' "$metadata_check_output"', count=1), 'Build workflow Fetch PGO Profdata must include: printf \'%s\\n\' "$metadata_check_output" | sed \'s/^::warning::/PGO fallback probe warning: /\'', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '              local is_cpu_specific_profdata=false\n', ''), 'Build workflow Fetch PGO Profdata must include: local is_cpu_specific_profdata=false', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '                is_cpu_specific_profdata=true\n', ''), 'Build workflow Fetch PGO Profdata must include: is_cpu_specific_profdata=true', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '              local required_ffmpeg_cache_suffix=pgo-v1-clang\n', ''), 'Build workflow Fetch PGO Profdata must include: local required_ffmpeg_cache_suffix=pgo-v1-clang', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '              if [ "$expected_cpu" != x86-64 ]; then\n                is_cpu_specific_profdata=true\n                required_ffmpeg_cache_suffix=profiling-v1-clang\n              fi\n', '              if [ "$expected_cpu" != x86-64 ]; then\n                is_cpu_specific_profdata=true\n                required_ffmpeg_cache_suffix=pgo-v1-clang\n              fi\n'), 'Build workflow Fetch PGO Profdata must include: required_ffmpeg_cache_suffix=profiling-v1-clang', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '--required-ffmpeg-cache-suffix="$required_ffmpeg_cache_suffix"', '--required-ffmpeg-cache-suffix=pgo-v1-clang'), 'Build workflow Fetch PGO Profdata must include: --required-ffmpeg-cache-suffix="$required_ffmpeg_cache_suffix"', 'pgo-fetch-scope'),
            ...
            case(lambda repo: replace_text(build_workflow(repo), '                --expected-cpu="$expected_cpu"\n', ''), 'Build workflow Fetch PGO Profdata must include: --expected-cpu="$expected_cpu"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '              if [ "$is_cpu_specific_profdata" = true ]; then\n                metadata_check_args+=(--require-target-cpu)\n              fi\n', ''), 'Build workflow Fetch PGO Profdata must include: if [ "$is_cpu_specific_profdata" = true ]; then', 'pgo-fetch-scope'),
            ...
            case(lambda repo: replace_text(build_workflow(repo), '                metadata_check_args+=(--current-toolchain="$current_toolchain")\n', ''), 'Build workflow Fetch PGO Profdata must include: metadata_check_args+=(--current-toolchain="$current_toolchain")', 'pgo-fetch-scope'),
            ...
            case(lambda repo: replace_text(build_workflow(repo), '              return 1\n            fi\n\n            if ! git show "FETCH_HEAD:${source_path}" > "$destination" 2>/dev/null; then', '            fi\n\n            if ! git show "FETCH_HEAD:${source_path}" > "$destination" 2>/dev/null; then', count=1), 'Build workflow Fetch PGO Profdata must skip branches with missing metadata before copying profdata', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '          profdata_branch_for() {', '          profdata_branch_for_disabled() {', count=1), 'Build workflow Fetch PGO Profdata must include: profdata_branch_for() {', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '          fetch_profdata_with_fallback() {', '          fetch_profdata_no_fallback() {', count=1), 'Build workflow Fetch PGO Profdata must include: fetch_profdata_with_fallback() {', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '            target_branch=$(profdata_branch_for "$target_cpu" "$target")', '            target_branch=$(profdata_branch_for x86-64 "$target")', count=1), 'Build workflow Fetch PGO Profdata must include: target_branch=$(profdata_branch_for "$target_cpu" "$target")', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '            if [ "$target_branch" = "$baseline_branch" ]; then', '            if false; then', count=1), 'Build workflow Fetch PGO Profdata must include: if [ "$target_branch" = "$baseline_branch" ]; then', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '            if fetch_profdata "$target" "$target_cpu" "$target_branch" "$source_path" false; then', '            if fetch_profdata "$target" "$target_cpu" "$target_branch" "$source_path"; then', count=1), 'Build workflow Fetch PGO Profdata must include: if fetch_profdata "$target" "$target_cpu" "$target_branch" "$source_path" false; then', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '            echo "Falling back to x86-64 PGO profdata: target=${target} branch=${baseline_branch}"', '            echo "No fallback"', count=1), 'Build workflow Fetch PGO Profdata must include: echo "Falling back to x86-64 PGO profdata: target=${target} branch=${baseline_branch}"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '            fetch_profdata_with_fallback 8b-lib x265.profdata || true', '            fetch_profdata 8b-lib profdata-x86-64-8b-lib x265.profdata', count=1), 'Build workflow Fetch PGO Profdata must include: fetch_profdata_with_fallback 8b-lib x265.profdata || true', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '          fetch_profdata_with_fallback all x265.profdata || true', '          fetch_profdata all profdata-x86-64-all x265.profdata', count=1), 'Build workflow Fetch PGO Profdata must include: fetch_profdata_with_fallback all x265.profdata || true', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '            append_pgo_status 8b-lib skipped push "push path only consumes all-target profdata"', '            fetch_profdata 8b-lib profdata-x86-64-8b-lib x265.profdata', count=1), 'Build workflow Fetch PGO Profdata must include: append_pgo_status 8b-lib skipped push "push path only consumes all-target profdata"', 'pgo-fetch-scope'),
            case(lambda repo: replace_text(build_workflow(repo), '          resolve_cpu_march_flag() {', '          resolve_cpu_march_flag_disabled() {', count=1), 'Build workflow Compile X265 must include CPU-specific CXX flags: resolve_cpu_march_flag() {', 'build-compile-cpu-flags'),
            case(lambda repo: replace_text(build_workflow(repo), '          CPU_CXX_FLAG=$(resolve_cpu_march_flag "$CPU")', '          CPU_CXX_FLAG=""', count=1), 'Build workflow Compile X265 must include CPU-specific CXX flags: CPU_CXX_FLAG=$(resolve_cpu_march_flag "$CPU")', 'build-compile-cpu-flags'),
            case(lambda repo: replace_text(build_workflow(repo), '            BASE_CXX_FLAGS="${BASE_CXX_FLAGS} ${CPU_CXX_FLAG}"', '            echo "$CPU_CXX_FLAG"', count=1), 'Build workflow Compile X265 must include CPU-specific CXX flags: BASE_CXX_FLAGS="${BASE_CXX_FLAGS} ${CPU_CXX_FLAG}"', 'build-compile-cpu-flags'),
            ...
            case(
                ...
                'Build workflow Setup Shared Dependencies (Runtime Smokes) must pin ffmpeg-cache-suffix=lavf-cli-v5-clang',
                'windows-dependency-smoke-scope',
            ),
            case(lambda repo: replace_text(build_workflow(repo), '          ffmpeg-cache-suffix: lavf-cli-v5-clang', '          ffmpeg-cache-suffix: lavf-v4-clang', count=1), 'Build workflow runtime-smoke FFmpeg cache suffix must differ from warning-scan library-only cache suffix', 'windows-dependency-smoke-scope'),
            case(
                lambda repo: replace_text(
                    build_workflow(repo),
                    '          ffmpeg-cache-suffix: lavf-v4-clang',
                ...
                'Build workflow Setup Shared Dependencies (Build Only) must pin ffmpeg-cache-suffix=lavf-v4-clang',
                'windows-dependency-smoke-scope',
            ),
            ...
            case(lambda repo: replace_text(build_workflow(repo), '              --enable-indev=lavfi --enable-filter=testsrc2,scale', '              --enable-indev=lavfi --enable-filter=testsrc2', count=1), 'Build workflow runtime-smoke FFmpeg config must enable dependency: --enable-filter=testsrc2,scale', 'windows-dependency-smoke-scope'),
            ...
            case(lambda repo: replace_text(runtime_suite(repo), "grep -Fq 'Incorrect video-signal-type-preset, aborting' smoke_vst_oversized.log", "grep -Fq 'video-signal-type-preset' smoke_vst_oversized.log"), 'Video-signal-type-preset oversized smoke must require malformed preset log', 'video-signal-type-preset-oversized-smoke'),
