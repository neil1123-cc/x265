...
FORBIDDEN_SNIPPET = 'OPT("vbv-maxrate") svtHevcParam->vbvMaxrate = (uint32_t)x265_atoi(value, bError);'
...
def check_repo(repo_root):
    ...
    for snippet in REQUIRED_SNIPPETS:
        if snippet not in svt_param_text:
            failures.append((TARGET.as_posix(), 0, f'missing SVT vbv-maxrate guardrail: {snippet}'))
...
def main():
    parser = argparse.ArgumentParser(description='Check vbv-maxrate parse safety guardrails')
