...
def check_repo(repo_root):
    ...
    failures = []
    if 'svtHevcParam->vbvBufsize = (uint32_t)x265_atoi(value, bError);' in svt_param_text:
    ...
    for snippet in REQUIRED_SNIPPETS:
        if snippet not in parse_block:
            failures.append((TARGET.as_posix(), 0, f'missing SVT vbv-bufsize guardrail in parse block: {snippet}'))
...
def main():
    parser = argparse.ArgumentParser(description='Check SVT vbv-bufsize parse safety guardrails')
