...
FFMPEG_TAG_RE = re.compile(r'^n[0-9]+\.[0-9]+$')
DEPENDENCY_RULES = (
    {
        'name': 'L-SMASH',
        'cache_key': 'lsmash',
...
DEPS_CACHE_REF_RULES = (
    {
        'name': 'FFmpeg',
        'cache_key': 'ffmpeg',
        'action_field': 'ffmpeg-ref',
        'tag_pattern': FFMPEG_TAG_RE,
        'latest_step_id': 'ffmpeg',
        'latest_api': 'https://api.github.com/repos/FFmpeg/FFmpeg/tags?per_page=100',
        'update_ref_snippet': 'sed -i "0,/default: n[0-9]\\+\\.[0-9]\\+/s//default: ${{ steps.ffmpeg.outputs.tag }}/" "$action"',
    },
    {
        'name': 'mimalloc',
        'cache_key': 'mimalloc',
        'action_field': 'mimalloc-ref',
        'tag_pattern': SEMVER_TAG_RE,
        'latest_step_id': 'mimalloc',
        'latest_api': 'https://api.github.com/repos/microsoft/mimalloc/tags?per_page=100',
        'update_ref_snippet': 'sed -i "0,/default: v[0-9]\\+\\.[0-9]\\+\\.[0-9]\\+/s//default: ${{ steps.mimalloc.outputs.tag }}/" "$action"',
    },
    {
        'name': 'Obuparse',
        'cache_key': 'obuparse',
        'action_field': 'obuparse-ref',
        'tag_pattern': SEMVER_TAG_RE,
        'latest_step_id': 'obuparse',
        'latest_api': 'https://api.github.com/repos/dwbuiten/obuparse/tags?per_page=100',
        'update_ref_snippet': 'sed -i "/obuparse-ref:/,/extra-msys2-packages:/s/default: v[0-9]\\+\\.[0-9]\\+\\.[0-9]\\+/default: ${{ steps.obuparse.outputs.tag }}/" "$action"',
    },
    *DEPENDENCY_RULES,
)
TAG_CACHE_REF_RULES = tuple(rule for rule in DEPS_CACHE_REF_RULES if 'tag_pattern' in rule)
...
def validate_tag(value, description, path, pattern):
    if not isinstance(value, str) or not pattern.fullmatch(value):
        fail(f'{description} must be a pinned release tag, got {value!r}', path)
...
def validate_deps_cache_refs(repo_root, allow_missing_cache=False):
    cache_path = repo_root / DEPS_CACHE_PATH
    if not cache_path.is_file():
        if allow_missing_cache:
            print(f'::warning::{DEPS_CACHE_PATH.as_posix()} is missing; allowed by --allow-missing-cache')
            return
        fail(f'missing dependency cache provenance file: {DEPS_CACHE_PATH.as_posix()}', cache_path)
        return
    ...
    for rule in DEPS_CACHE_REF_RULES:
        ...
        if tag_pattern is not None:
            validate_tag(actual, f'{DEPS_CACHE_PATH.as_posix()} {cache_key}', cache_path, tag_pattern)
        else:
...
def validate_update_deps_provenance(repo_root):
    ...
    for rule in TAG_CACHE_REF_RULES:
        required = (
            rule['latest_api'],
            f'id: {rule["latest_step_id"]}',
            rule['update_ref_snippet'],
            f'"{rule["cache_key"]}": "${{{{ steps.{rule["latest_step_id"]}.outputs.tag }}}}"',
        )
    ...
    for snippet in (
        ...
        if snippet not in text:
            fail(f'update-deps workflow is missing dependency provenance snippet: {snippet}', update_path)
...
def main():
    ...
    validate_deps_cache_refs(repo_root, args.allow_missing_cache)
