...
def check_repo(repo_root):
    ...
    failures = []
    if 'OPT("qpmin") svtHevcParam->minQpAllowed = x265_atoi(value, bError);' in svt_param_text:
        failures.append((TARGET.as_posix(), 0, 'forbidden SVT qpmin regression: invalid values must not overwrite prior state'))
        return failures
    ...
    parse_block = extract_option_block('qpmin', 'svtHevcParam->minQpAllowed = minQpAllowed;')
    for snippet in REQUIRED_SNIPPETS:
        if snippet not in parse_block:
            failures.append((TARGET.as_posix(), 0, f'missing SVT qpmin guardrail in parse block: {snippet}'))
    parse_pos = parse_block.find('int minQpAllowed = parseOptionIntValue(value, bMinQpAllowedError);')
    error_pos = parse_block.find('bError |= bMinQpAllowedError;', parse_pos if parse_pos != -1 else 0)
    guard_pos = parse_block.find('if (!bMinQpAllowedError)', error_pos if error_pos != -1 else 0)
    assign_pos = parse_block.find('svtHevcParam->minQpAllowed = minQpAllowed;', guard_pos if guard_pos != -1 else 0)
    if -1 in (parse_pos, error_pos, guard_pos, assign_pos) or not (parse_pos < error_pos < guard_pos < assign_pos):
        failures.append((TARGET.as_posix(), 0, 'SVT qpmin parse block must gate assignment on parseOptionIntValue success'))
    return failures


def main():
    parser = argparse.ArgumentParser(description='Check SVT qpmin parse safety guardrails')
    ...
    print('SVT qpmin parse safety validated')
