...
def check_repo(repo_root):
    ...
    for snippet in REQUIRED_SNIPPETS:
        if snippet not in text:
            failures.append((TARGET.as_posix(), 0, f'missing abr allocBuffers analysisRead guardrail: {snippet}'))
    ...
    def has_guarded_analysis_read(block):
        alloc_pos = block.find('m_analysisRead[pass] = new (std::nothrow) ThreadSafeInteger[m_queueSize];')
        guard_pos = block.find('if (!m_analysisRead[pass])', alloc_pos if alloc_pos != -1 else 0)
    ...
    if not has_guarded_analysis_read(branch1_text) or not has_guarded_analysis_read(branch2_text):
        failures.append((TARGET.as_posix(), 0, 'AbrEncoder::allocBuffers must guard m_analysisRead[pass] allocation in both queue setup branches'))

    return failures


def main():
    parser = argparse.ArgumentParser(description='Check AbrEncoder allocBuffers analysisRead allocation guards')
    ...
    print('AbrEncoder allocBuffers analysisRead guards validated')
