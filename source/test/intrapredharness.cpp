...
IntraPredHarness::IntraPredHarness()
{
    for (int i = 0; i < INPUT_SIZE; i++)
        pixel_buff[i] = std::rand() % PIXEL_MAX;
...
#if _DEBUG
    std::fill_n(reinterpret_cast<uint8_t*>(pixel_out_vec), sizeof(pixel_out_vec), uint8_t(0xCD));
    std::fill_n(reinterpret_cast<uint8_t*>(pixel_out_c), sizeof(pixel_out_c), uint8_t(0xCD));
#endif

    for (int i = 0; i <= 100; i++)
    {
        int rand_filter = std::rand() & 1;
        if (width > 16)
            rand_filter = 0;
...
bool IntraPredHarness::check_angular_primitive(const intra_pred_t ref[], const intra_pred_t opt[], int sizeIdx)
{
    ...
    for (int i = 0; i <= 100; i++)
    {
        int bFilter = (width <= 16) && (std::rand() % 2);
        for (int pmode = 2; pmode <= 34; pmode++)
        {
            ...
            for (int k = 0; k < width; k++)
            {
                if (std::memcmp(pixel_out_vec + k * FENC_STRIDE, pixel_out_c + k * FENC_STRIDE, width * sizeof(pixel)))
                {
                    std::printf("ang_%dx%d, Mode = %d, Row = %d failed !!\n", width, width, pmode, k);
                    ref[pmode](pixel_out_c, stride, pixel_buff + j, pmode, bFilter);
...
#if _DEBUG
    std::fill_n(reinterpret_cast<uint8_t*>(pixel_out_33_vec), sizeof(pixel_out_33_vec), uint8_t(0xCD));
    std::fill_n(reinterpret_cast<uint8_t*>(pixel_out_33_c), sizeof(pixel_out_33_c), uint8_t(0xCD));
#endif
...
{
    std::fill_n(reinterpret_cast<uint8_t*>(pixel_out_c), sizeof(pixel_out_c), uint8_t(0));
    std::fill_n(reinterpret_cast<uint8_t*>(pixel_out_vec), sizeof(pixel_out_vec), uint8_t(0));
    int j = 0;
...
bool IntraPredHarness::testCorrectness(const EncoderPrimitives& ref, const EncoderPrimitives& opt)
{
    for (int i = BLOCK_4x4; i <= BLOCK_32x32; i++)
    {
        ...
        if (opt.cu[i].intra_pred[PLANAR_IDX])
        {
            ...
            {
                std::printf("intra_planar %dx%d failed\n", size, size);
                return false;
            }
        }
        if (opt.cu[i].intra_pred[DC_IDX])
        {
            ...
            {
                std::printf("intra_dc %dx%d failed\n", size, size);
                return false;
            }
        }
        ...
        {
            std::printf("intra_angular failed\n");
            return false;
        }
        ...
        if (opt.cu[i].intra_pred_allangs)
        {
            ...
            {
                std::printf("intra_allangs failed\n");
                return false;
            }
        }
        if (opt.cu[i].intra_filter)
        {
            ...
            {
                std::printf("intra_filter_%dx%d failed\n", size, size);
                return false;
            }
        }
    }
...
void IntraPredHarness::measureSpeed(const EncoderPrimitives& ref, const EncoderPrimitives& opt)
{
    ...
    for (int i = BLOCK_4x4; i <= BLOCK_32x32; i++)
    {
        ...
        if (opt.cu[i].intra_pred[PLANAR_IDX])
        {
            std::printf("intra_planar_%dx%d", size, size);
        ...
        if (opt.cu[i].intra_pred[DC_IDX])
        {
            std::printf("intra_dc_%dx%d[f=0]", size, size);
            REPORT_SPEEDUP(opt.cu[i].intra_pred[DC_IDX], ref.cu[i].intra_pred[DC_IDX],
                pixel_out_vec, FENC_STRIDE, pixel_buff + srcStride, 0, 0);
            if (size <= 16)
            {
                std::printf("intra_dc_%dx%d[f=1]", size, size);
                REPORT_SPEEDUP(opt.cu[i].intra_pred[DC_IDX], ref.cu[i].intra_pred[DC_IDX],
        ...
        if (opt.cu[i].intra_pred_allangs)
        {
            ...
            std::printf("intra_allangs%dx%d", size, size);
        ...
        for (int mode = 2; mode <= 34; mode += 1)
        {
            if (opt.cu[i].intra_pred[mode])
            {
                ...
                refLeft[0] = refAbove[0];
                std::printf("intra_ang_%dx%d[%2d]", width, width, mode);
        ...
        if (opt.cu[i].intra_filter)
        {
            std::printf("intra_filter_%dx%d", size, size);
